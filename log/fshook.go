...
func NewFSHook(path string, formatter logrus.Formatter, rotSched RotationScheduler) logrus.Hook {
	if formatter == nil {
		formatter = &logrus.JSONFormatter{}
	}
	hook := &fsHook{
		entries:   make(chan logrus.Entry, 1024),
		path:      path,
		formatter: formatter,
		scheduler: rotSched,
	}
...
func (hook *fsHook) writeEntry(entry *logrus.Entry) error {
